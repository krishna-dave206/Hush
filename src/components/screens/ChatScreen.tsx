...
import { ArrowLeft, Send, ShieldAlert, UserCheck, MapPin, MoreHorizontal, AlertTriangle } from 'lucide-react';
...
export const ChatScreen: React.FC<{ chatId?: string; recipientId?: string; navigateTo: (screen: ScreenType, params?: any) => void }> = ({ chatId, recipientId, navigateTo }) => {
  const { user } = useAuth();
  ...
  const handleReveal = () => {
    ...
    socket?.emit('send-message', { roomId: chatId || `chat_${user!.id}_${mockRecipient.id}`, ...newMsg });
  };
    ...
    <div className="h-screen bg-zinc-950 flex flex-col pt-12">
      ...
      <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-zinc-950/80 backdrop-blur-xl sticky top-0 z-10">
        <div className="flex items-center gap-4">
          <button onClick={() => navigateTo('HOME')} className="w-10 h-10 flex items-center justify-center transition-all bg-zinc-900 border border-white/5 rounded-xl">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div className="flex items-center gap-3">
            <Avatar className="w-10 h-10 rounded-xl border border-white/10">
              <AvatarFallback className="bg-orange-500/10 text-orange-500 font-bold">{mockRecipient.handle[1].toUpperCase()}</AvatarFallback>
            </Avatar>
            <div>
              <h2 className="font-bold text-sm tracking-tight">{mockRecipient.handle}</h2>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Active Nearby</span>
              </div>
            </div>
          </div>
        </div>
        <button onClick={() => setIsPanicOpen(true)} className="w-10 h-10 flex items-center justify-center bg-red-500/10 text-red-500 rounded-xl border border-red-500/20 active:scale-90 transition-all">
      ...
      <ScrollArea className="flex-1 px-6">
        <div ref={scrollRef} className="py-8 flex flex-col gap-6">
          <div className="flex flex-col items-center justify-center text-center px-8 mb-8">
            ...
            <h4 className="text-sm font-bold text-zinc-400 mb-1 tracking-tight">Encryption Active</h4>
            <p className="text-[10px] text-zinc-600 uppercase tracking-[0.2em] font-medium max-w-[200px]">Your identity is hidden until you choose to reveal.</p>
          </div>
            ...
            >
              {msg.type === 'reveal_confirmed' ? (
                <div className="bg-orange-500/10 border border-orange-500/20 p-4 rounded-3xl max-w-[80%] flex items-center gap-3">
                   ...
                   <div>
                      <p className="text-xs font-bold text-orange-500 uppercase tracking-widest mb-1">Location Shared</p>
                      <p className="text-sm text-white/90">Meet at Table #4</p>
                   </div>
                </div>
              ) : (
                <div className={`max-w-[80%] px-5 py-3 rounded-[24px] text-sm leading-relaxed shadow-sm ${
              ...
              <span className="text-[9px] font-bold text-zinc-600 tracking-widest mt-1.5 flex items-center gap-1 uppercase">
                ...
                {msg.senderId === user?.id && <span className="opacity-50">· Sent</span>}
              </span>
            </motion.div>
          ))}
        </div>
      ...
      <Dialog open={isPanicOpen} onOpenChange={setIsPanicOpen}>
        <DialogContent className="bg-zinc-900 border-red-500/20 rounded-[32px] max-w-[90vw] mx-auto sm:max-w-md">
          <DialogHeader className="flex flex-col items-center text-center space-y-4">
            ...
            <DialogTitle className="text-2xl font-bold tracking-tight text-white">Panic Exit?</DialogTitle>
            <DialogDescription className="text-zinc-400 text-base">
              This will instantly end the chat, block this user for the session, and clear your presence from the venue.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex flex-col gap-3 sm:flex-col mt-4">
            <Button onClick={handlePanicExit} className="h-14 bg-red-600 hover:bg-red-700 text-white font-bold rounded-2xl w-full">Yes, Terminate Session</Button>
            <Button variant="ghost" onClick={() => setIsPanicOpen(false)} className="text-zinc-500 font-bold w-full uppercase text-xs tracking-widest">Cancel</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isRevealOpen} onOpenChange={setIsRevealOpen}>
        <DialogContent className="bg-zinc-900 border-white/10 rounded-[32px] max-w-[90vw] mx-auto sm:max-w-md">
          <DialogHeader className="flex flex-col items-center text-center space-y-4">
            ...
            <DialogTitle className="text-2xl font-bold tracking-tight text-white">Reveal Identity?</DialogTitle>
            <DialogDescription className="text-zinc-400 text-base">
              This will show your table number and profile photo to this user. Only reveal if you feel safe meeting.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex flex-col gap-3 sm:flex-col mt-4">
            <Button onClick={handleReveal} className="h-14 bg-white text-black hover:bg-zinc-200 font-bold rounded-2xl w-full">I'm ready to meet</Button>
            <Button variant="ghost" onClick={() => setIsRevealOpen(false)} className="text-zinc-500 font-bold w-full uppercase text-xs tracking-widest text-[10px]">Stay Anonymous</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
